...
ADMIN_ID, RENDER_URL = 8086545587, "https://mafia-bot-1-cfws.onrender.com"
...
def load_db():
    ...
    except Exception as e: print(f"⚠️ Baza yuklanmadi: {e}")
...
def save_db():
    ...
    except Exception as e: print(f"⚠️ Saqlanmadi: {e}")

def check_user(uid, name="Foydalanuvchi"):
    if uid not in DB["users"]:
        DB["users"][uid] = {"name": name, "balance": 10000, "tickets": 5, "last_bonus": 0, "apple_game": None, "aviator_game": None, "mines_game": None, "state": None, "temp_bet": None}
        save_db()
    u = DB["users"][uid]
...
def callback_handler(call):
    ...
    if call.data == "to_main":
        ...
        txt = f"👑 *SHOX SUPREME PLATFORMA v9.0*\n\n💵 *Balans:* {ud['balance']} so'm\n🎫 *Chiptalar:* {ud['tickets']} ta"
        try: bot.edit_message_text(txt, cid, mid, parse_mode="Markdown", reply_markup=get_main_keyboard(uid))
        except: pass

    elif call.data == "get_daily_bonus":
        t_now = int(time.time())
        if t_now - ud.get("last_bonus", 0) < 86400:
            rem = 86400 - (t_now - ud["last_bonus"])
            bot.answer_callback_query(call.id, f"❌ Bonus olingan! Keyingi bonusga {rem//3600} soat bor.", show_alert=True)
            return
        ...
        try: bot.edit_message_text(f"👑 *SHOX SUPREME PLATFORMA v9.0*\n\n💵 *Balans:* {ud['balance']} so'm\n🎫 *Chiptalar:* {ud['tickets']} ta", cid, mid, parse_mode="Markdown", reply_markup=get_main_keyboard(uid))
        except: pass
        ...
        # Koeffitsiyent hisoblash qismi
        total_cells, coeff = 30, 1.0
        ...
        for r in range(13):
            ...
            grid.append(items)
        ud["apple_game"] = {"grid": grid, "current_row": 0, "bet": bet, "payout": bet}; save_db(); show_apple(call.message, ud)
        ...
        if ag["grid"][ag["current_row"]][idx] == "bad":
            ...
            try: bot.edit_message_text("💀 *Chirigan olma! Mag'lubiyat.*", cid, mid, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🍏 Qayta o'ynash", callback_data="prep_apple")))
            except: pass
            return
        ...
        try: bot.edit_message_text("💰 Olma o'yinidan pul yechib olindi!", cid, mid, reply_markup=get_main_keyboard(uid))
        except: pass
        ...
        if not bet or bet > ud["balance"]: return
        ud["balance"] -= bet
        crash = DB["settings"].get("next_aviator") or round(random.uniform(1.10, 4.50), 2)
        ...
        try: bot.edit_message_text(f"👑 *ADMIN PANEL*\n\n👥 Foydalanuvchilar: {len(DB['users'])} ta\n💰 Jami: {sum([u.get('balance',0) for u in DB['users'].values()])} so'm", cid, mid, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅ Bosh Menyu", callback_data="to_main")))
        except: pass
...
def show_apple(mo, ud):
    ...
    for ri in range(12, -1, -1):
        ...
        for ci in range(5): r_btns.append(types.InlineKeyboardButton("🍏" if ri < ag["current_row"] else "🟫" if ri == ag["current_row"] else "🔒", callback_data=f"ap_select_{ci}" if ri == ag["current_row"] else "lock"))
        kb.row(*r_btns)
