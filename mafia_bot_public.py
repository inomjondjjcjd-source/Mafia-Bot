...
def check_user(uid, name="Foydalanuvchi"):
    if uid not in DB["users"]:
        DB["users"][uid] = {
            ...
            "balance": 10000,
            "tickets": 5,
            "last_bonus": 0, # Kundalik bonus uchun vaqt xotirasi
            "apple_game": None,
            "aviator_game": None,
            "mines_game": None,
            "state": None,
        ...
        save_db()
    u = DB["users"][uid]
    # Yangi maydonlar qo'shilganda xato bermasligi uchun tekshirish
...
def start_cmd(message):
    ...
    txt = (
        ...
        f"⚡️ *Bot 10x tezlashtirildi. Hamma o'yinlar Inline tugmalarga o'tkazildi!*"
    )
...
def callback_handler(call):
    ...
    # --- 1-YANGILIK: KUNDALIK BONUS ---
        ...
        if current_time - ud["last_bonus"] < 86400:
            ...
            hours = rem // 3600
            mins = (rem % 3600) // 60
            bot.answer_callback_query(call.id, f"❌ Siz bonusni olgansiz! Keyingi bonusga {hours} soat, {mins} daqiqa bor.", show_alert=True)
            return
        ...
        save_db()
        bot.answer_callback_query(call.id, f"🎁 Tabriklaymiz! Balansingizga {bonus_amount} so'm bonus qo'shildi!", show_alert=True)
        txt = f"👑 *SHOX SUPREME PLATFORMA v9.0*\n\n💵 *Balans:* {ud['balance']} so'm\n🎫 *Chiptalar:* {ud['tickets']} ta"
        try: bot.edit_message_text(txt, call.message.chat.id, call.message.message_id, parse_mode="Markdown", reply_markup=get_main_keyboard(uid))
        except: pass
    ...
    # --- APPLE OF FORTUNE (INLINE TUGMALI) ---
        ...
        kb.add(types.InlineKeyboardButton("⬅️ Chiqish", callback_data="to_main"))
        try: bot.edit_message_text("🍏 *APPLE OF FORTUNE*\n\nTikish summasini tugma orqali tanlang:", call.message.chat.id, call.message.message_id, parse_mode="Markdown", reply_markup=kb)
        except: pass
    ...
    # --- AVIATOR (INLINE TUGMA VA AUTO CASHOUT TIZIMI) ---
        ...
        # 2-YANGILIK: Auto cash-out tugmalari
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(
        ...
        try: bot.edit_message_text(f"🚀 Tikilgan: *{bet} so'm*\n\nAvtomatik naqdlashtirish (Auto Cash-out) koeffitsiyentini tanlang:", call.message.chat.id, call.message.message_id, parse_mode="Markdown", reply_markup=kb)
        except: pass
        ...
        # Samolyot parvozi (Tez va yengil asinxron oqim)
...
def show_mines_board(message_obj, ud, lost=False, won=False):
    ...
    if not lost and not won:
        ...
        kb.add(types.InlineKeyboardButton("⬅ Honor / Chiqish", callback_data="to_main"))
    else:
...
# --- AVIATOR PARVOZ DASTURI (OPTIMIZATSIYA QILINGAN) ---
def run_aviator_thread(chat_id, message_id, uid):
    for _ in range(60):
        ...
        # Auto Cash-out tekshirish (1-yangilik logikasi)
