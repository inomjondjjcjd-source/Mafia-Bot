...
TOKEN = "8691200742:AAFXoXv_45iqMkpjCERCkhiJnByK3GKybY0"
ADMIN_ID = 8086545587
...
def cb(call):
    ...
    if d=="to_main":
        ...
        edit(f"👑 *ADMIN — {BOT_NAME}*\n\n👥 {len(DB['users'])} ta user\n💵 Umumiy: {sum(u.get('balance',0) for u in DB['users'].values()):,} so'm\n🎟 Promokodlar: {len(DB.get('promocodes',{}))} ta",kb)
    elif d=="adm_set_kf" and uid==ADMIN_ID:
        ud["state"]="set_kf"; save_db(); edit("🚀 Keyingi Aviator kf'ni yozing (masalan: 3.45):")
    elif d=="adm_add_money" and uid==ADMIN_ID:
        ud["state"]="add_money"; save_db(); edit("💰 `ID summa` yozing:",parse_mode="Markdown")
    elif d=="adm_remove_money" and uid==ADMIN_ID:
        ud["state"]="remove_money"; save_db(); edit("💸 `ID summa` yozing:",parse_mode="Markdown")
    elif d=="adm_create_promo" and uid==ADMIN_ID:
        ud["state"]="create_promo"; save_db(); edit("🎟 *Promokod yaratish*\n\n`KOD SUMMA` yoki faqat `SUMMA`\nMasalan: `VIP2024 50000`",parse_mode="Markdown")
        ...
        kb=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🗑 Hammasini o'chir",callback_data="adm_clear_promos"),types.InlineKeyboardButton("⬅ Admin",callback_data="admin_dashboard"))
        edit("\n".join(lines),kb)
    elif d=="adm_clear_promos" and uid==ADMIN_ID:
        DB["promocodes"]={}; save_db()
        bot.answer_callback_query(call.id,"🗑 O'chirildi!",show_alert=True)
        edit("✅ Tozalandi.",back_kb("admin_dashboard"))
        ...
        ud["state"]="enter_promo"; save_db()
        edit("🎟 *PROMOKOD*\n\nKodingizni yozing:",back_kb())elif d=="prep_mines":
        ...
        if ag["grid"][ag["current_row"]][idx]=="bad":
            ...
            edit("💀 *Chirigan olma! Yutqazdingiz!*",types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🍏 Qayta",callback_data="prep_apple"),types.InlineKeyboardButton("⬅ Menyu",callback_data="to_main"))); return
...
def run_aviator(chat_id, message_id, uid):
    for _ in range(60):
        ...
        if ag["current_win"]>=ag["crash"]:
            ...
            try: bot.edit_message_text(f"💥 *BOOM! x{ag['crash']} da portladi!*\n💵 Balans: *{ud['balance']:,} so'm*",chat_id,message_id,parse_mode="Markdown",reply_markup=back)
            except: pass
            break
        kb=types.InlineKeyboardMarkup()
        if ag["auto_co"]: kb.add(types.InlineKeyboardButton(f"🎯 Auto: x{ag['auto_co']} | Hozir: x{ag['current_win']}",callback_data="lock"))
