...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bal_str = "Cheksiz" if user.id == ADMIN_ID else str(db_user["balance"])
    armor_str = "Mavjud" if db_user["has_armor"] else "Yo'q"
        
    text = (
        "True Mafia Botiga Xush Kelibsiz!\n\n"
...
async def promo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Format noto'g'ri. Ishlatish: /promokod 255500")
        return
    ...
    if user_id != ADMIN_ID:
        db_user["balance"] += 500
    await update.message.reply_text("Promo-kod faollashdi! Balansingizga 500 olmos qo'shildi.")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "open_shop":
        ...
        kb = [
            ...
            [InlineKeyboardButton("Komissar (40)", callback_data="buy_cop"), InlineKeyboardButton("Zirh (70)", callback_data="buy_armor")],
            [InlineKeyboardButton("Orqaga", callback_data="back_to_main")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb))
        
    elif query.data == "back_to_main":
        bal_str = "Cheksiz" if u_id == ADMIN_ID else str(db_user["balance"])
        armor_str = "Mavjud" if db_user["has_armor"] else "Yo'q"
        ...
        if item == "armor":
            ...
            await query.edit_message_text("Maxsus Zirh muvaffaqiyatli sotib olindi!",
                                          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Orqaga", callback_data="back_to_main")]]))
        else:
            db_user["selected_role"] = roles[item]
            await query.edit_message_text(f"Keyingi o'yin uchun rolingiz {roles[item]} qilib belgilandi!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Orqaga", callback_data="back_to_main")]]))
        ...
        await query.edit_message_text("So'rov adminga yuborildi!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Orqaga", callback_data="back_to_main")]]))
...
async def start_mafia_game(g_id, context):
    ...
    for p_id, p_data in GAMES[g_id]["players"].items():
        try:
            await context.bot.send_message(chat_id=p_id, text=f"O'yin boshlandi! Sizning rolingiz: {p_data['role']}")
        except Exception:
            pass
...
async def run_day(g_id, context):
    ...
    await context.bot.send_message(chat_id=g_id, text=f"Ertalab bo'ldi!\n\nTunda o'ldirildi: {killed_name}")
    
    if await check_game_end(g_id, context):
        return
        
    await context.bot.send_message(chat_id=g_id, text="Kunduzgi ovoz berish boshlandi! Gumonlanuvchiga shaxsiyda ovoz bering.")
...
async def resolve_day_vote(g_id, context):
    ...
    if not votes:
        await context.bot.send_message(chat_id=g_id, text="Hech kim ovoz bermadi.")
    else:
        counts = {}
        ...
        if len(winners) > 1:
            await context.bot.send_message(chat_id=g_id, text="Ovozlar teng. Hech kim qatl qilinmadi.")
        else:
            lynched = winners[0]
            GAMES[g_id]["players"][lynched]["alive"] = False
            await context.bot.send_message(chat_id=g_id, text=f"Qatl qilindi: {GAMES[g_id]['players'][lynched]['name']}")
